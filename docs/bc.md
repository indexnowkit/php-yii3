...
| **`ActiveRecord\IndexNowEvents`**, **`ActiveRecord\ObserverProvider`** (`set($observer, ?LoggerInterface $logger = null)`, `get()`, `isSet()`), **`ActiveRecord\IndexNowObserver`** public hooks | The attribute stays a drop-in; the observer's public hooks keep their names; `set()` grows by appended, defaulted parameters only. |
